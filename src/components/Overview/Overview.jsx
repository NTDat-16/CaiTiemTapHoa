...
export default function Overview() {
  ...
  const fetchCustomer = async () => {
    try {
      const response = await fetch(
        `${API_BASE}/Customer?PageNumber=1&PageSize=100`,
        {
          headers: { ...authHeader },
        }
      );
      ...
      const customersArray = Array.isArray(result?.data?.items)
        ? result.data.items
        : [];
      const customersStatus = customersArray.filter(
  ...
  const getTodayRevenue = async () => {
    try {
      ...
      const monthResponse = await fetch(
        `${API_BASE}/Reports/sales/overview?startDate=${monthStart}&endDate=${monthEnd}&groupBy=day`,
        { headers: { ...authHeader } }
      );
  ...
  const fetchOverview = async () => {
    if (!startDate) {
      message.error("Vui lòng chọn tháng cần xem báo cáo");
      return;
    }
    ...
    setLoading(true);
    try {
      await fetchChartData(monthStart, monthEnd);
    } catch (error) {
